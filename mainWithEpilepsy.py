...
from scaler import custom_scale_data, scale_data
...
meta_df = pd.read_csv('DataSetEpilepsy.csv')
...
epilepsy_array = [convertDF2MNE(i) for i in Epilepsy]
control_array = [convertDF2MNE(i) for i in Control]
...
patientsFeatures = calculate_features(epilepsy_array)
controlsFeatures = calculate_features(control_array)
