...
meta_df = pd.read_csv('EEGs.csv')
EP_sub = meta_df['subject.id'][meta_df['Group'] == 'Epilepsy']
CT_sub = meta_df['subject.id'][meta_df['Group'] == 'Control']
epilepsy = [pd.read_csv('EEGs.csv.gz'.format(i)) for i in EP_sub]
control = [pd.read_csv('EEGs.csv.gz'.format(i), compression='gzip') for i in CT_sub]
epilepsy = [i.iloc[:, 1:15] for i in epilepsy]
control = [i.iloc[:, 1:15] for i in control]


def get_psd_features(file_path):
    info = mne.create_info(list(file_path.columns), ch_types=['eeg'] * len(file_path.columns), sfreq=128)
    info.set_montage('standard_1020')
    data = mne.io.RawArray(file_path.T, info)
...
epilepsy_psd_array = [get_psd_features(i) for i in epilepsy]
control_psd_array = [get_psd_features(i) for i in control]
...
vgPerEpilepsy = construct_visibility_graph(epilepsy_psd_array)
vgPerControle = construct_visibility_graph(control_psd_array)
            ...
            for channel_idx in epoch_idx:
                ...
                for band_inx in channel_idx:
                    ...
                    vgband.append(np.array(
                        [avg_degree, max_degree, graph_density, radius, diameter, independence_number,
                         entropy, assortativity, clustering_coefficient, global_efficiency, max_clique_size]))
...
perBand = extract_graph_theory_features(vgPerEpilepsy)
perBandPerPatient = extract_graph_theory_features(vgPerControle)
epilepsy_epochs_labels = [len(i) * [0] for i in perBand]
controle_epochs_labels = [len(i) * [1] for i in perBandPerPatient]
data_list = perBand + perBandPerPatient
label_list = controle_epochs_labels + epilepsy_epochs_labels
